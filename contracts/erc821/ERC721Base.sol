...
contract ERC721Base is AssetRegistryStorage, IERC721Base, ERC165 {
  ...
  function getApprovedAddress(uint256 assetId) external view override returns (address) {
    return _getApprovedAddress(assetId);
  }
   ...
   * @return bool true if the asset has been approved by the holder
   */
  function isAuthorized(address operator, uint256 assetId) external view override returns (bool) {
   ...
   */
  function setApprovalForAll(address operator, bool authorized) external override {
   ...
   */
  function safeTransferFrom(address from, address to, uint256 assetId) external override {
    return _doTransferFrom(from, to, assetId, bytes(""), true);
  }
   ...
   */
  function safeTransferFrom(address from, address to, uint256 assetId, bytes calldata userData) external override {
   ...
   * @param assetId uint256 ID of the asset to be transferred
   */
  function transferFrom(address from, address to, uint256 assetId) external override virtual {
